...
# 🔑 APIキー設定
# ==========================================
GEMINI_API_KEY = "あなたのAPIキーと置き換えてください"
...
class OverlayUI:
  ...
  def __init__(self):
    ...
    self.root.attributes('-alpha', 0.85)  # 85%の半透明
    self.root.configure(bg='#1e1e1e')  # ダークモード風背景
    self.root.geometry('460x800+50+50')  # 幅460x高さ800 (左上に配置)
...
def capture_and_analyze():
  ...
  try:
    ...
    # 4. Gemini API 呼び出し
    prompt = (
        'あなたは最高のバーチャルツアーガイドです。このVR画像に写っている景色・場所について、以下の構成で400〜500文字程度で魅力的に解説してください。\n\n'
...
# ==========================================
# キーボードショートカットの監視（元の keyboard ライブラリ処理）
# ==========================================
def listen_keyboard():
  global target_side
  while True:
